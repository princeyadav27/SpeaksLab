...
import { deleteRecording, getRecordingBlob, getRecordings, saveRecording } from "@/lib/recordingsDb";
import {
...
import type { CloudRecordingMetadata } from "@/lib/recordingPaths";
...
/**
 * One row of the library. Cloud recordings arrive without media (lazy) and
 * local IndexedDB recordings keep their lazy-loading behavior too, so `blob`
 * is optional at rest — exactly like the original local-only library.
 */
type LibraryRecording = Omit<import("@/lib/recordingsDb").SavedRecording, "blob" | "metrics" | "evaluation"> & {
  blob?: Blob;
...
function stripBlob(recording: LibraryRecording): CloudRecordingMetadata {
  const rest = { ...recording } as Partial<LibraryRecording>;
  delete rest.blob;
  delete rest.storage;
  return { ...(rest as Omit<LibraryRecording, "blob" | "storage">), storage: "cloud" } as CloudRecordingMetadata;
}
...
export default function MyRecordingsClient() {
  ...
  useEffect(() => {
    // Load the account library first; local IndexedDB entries that are not in
    // the account yet are surfaced separately as device-only recordings.
    let cancelled = false;
    async function load() {
      const localItems = await getRecordings().catch(() => [] as import("@/lib/recordingsDb").SavedRecording[]);
      let cloudItems: CloudRecordingMetadata[] = [];
      try {
        cloudItems = await fetchCloudRecordings();
      } catch (error) {
        if (!cancelled) {
          setCloudError(error instanceof Error ? error.message : "Your account recordings could not be loaded.");
        }
      }
      if (cancelled) return;
      const cloudIds = new Set(cloudItems.map((item) => item.id));
      const deviceOnly = localItems
        .filter((item) => !cloudIds.has(item.id))
        .map((item): LibraryRecording => ({ ...item, storage: "local" }));
      const merged: LibraryRecording[] = [
        // Cloud metadata carries loosely-typed JSON (metrics/evaluation as
        // unknown); the library type narrows them back for rendering.
        ...cloudItems.map((item): LibraryRecording => ({
          ...item,
          storage: "cloud",
          metrics: item.metrics as LibraryRecording["metrics"],
          evaluation: item.evaluation as LibraryRecording["evaluation"],
        })),
        ...deviceOnly,
  ...
  useEffect(() => {
    ...
    if (!selected || selected.blob) return;
    ...
    const loader =
      selected.storage === "cloud" && selected.mediaUrl
        ? fetchRecordingMediaBlob(selected.mediaUrl)
        : getRecordingBlob(selectedId);
    void loader
      .then((blob) => {
        if (!blob) return;
  ...
  const deviceOnlyRecordings = useMemo(() => recordings?.filter((recording) => recording.storage === "local") ?? [], [recordings]);
  ...
  function updateRecord(updated: LibraryRecording) {
    setRecordings((current) => current?.map((item) => item.id === updated.id ? updated : item) ?? current);
  }

  /** Persists transcript/evaluation updates to the recording's own backend. */
  async function persistUpdate(recording: LibraryRecording): Promise<void> {
    if (recording.storage === "cloud") {
      await saveRecordingMetadata(stripBlob(recording));
    } else {
      await saveRecording(recording as import("@/lib/recordingsDb").SavedRecording);
    }
  }

  async function handleTranscribe(recording: LibraryRecording) {
    ...
    try {
      ...
      const updated: LibraryRecording = { ...recording, transcript, metrics: recording.metrics ?? calculateMetrics(transcript, recording.durationSeconds) };
  ...
  async function handleEvaluate(recording: LibraryRecording) {
  ...
  async function handleDelete(recording: LibraryRecording) {
    try {
      if (recording.storage === "cloud") {
        await deleteCloudRecording(recording.id);
      } else {
  ...
  /** Moves a device-only recording into the signed-in account. */
  async function handleUploadToDeviceCopy(recording: LibraryRecording) {
    if (!userId) {
      setActionError("Sign in to save recordings to your account.");
      return;
    }
    ...
    try {
      ...
      if (!blob) throw new Error("The recording media could not be loaded from this device.");
      ...
      // The recording now lives in the account; drop the device-only copy so
      // it does not resurface as a duplicate on the next visit.
      await deleteRecording(recording.id).catch(() => undefined);
      setRecordings((current) =>
        current?.map((item) => item.id === recording.id
          ? { ...item, storage: "cloud", mediaUrl: url, mediaPathname: pathname, blob }
          : item) ?? current,
      );
    } catch (error) {
      setActionError(error instanceof Error ? error.message : "The recording could not be saved to your account.");
    } finally {
      setUploadingId(null);
      setUploadProgress(0);
    }
  ...
  async function handleShare(recording: LibraryRecording) {
    try {
        ...
        <section className="mx-auto flex w-full max-w-[1280px] flex-1 flex-col px-5 pb-10 pt-7 sm:px-8 lg:px-10">
          <div className="flex flex-col gap-5 border-b border-ink/10 pb-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-[38rem] text-[13px] leading-relaxed text-ink/55">Review your practice, transcripts, and AI feedback — saved to your account and available on every device you sign in from.</p>
            </div>
          ...
          {deviceOnlyRecordings.length > 0 ? (
            <div className="mt-5 rounded-2xl border border-amber-900/20 bg-[#fdf6e7] p-4">
              <p className="text-[12px] font-medium text-[#7a5b18]">Saved on this device only</p>
              <p className="mt-1 text-[12px] leading-relaxed text-ink/55">These recordings were made before sign-in was introduced. Save them to your account to keep them across devices.</p>
              <ul className="mt-3 space-y-2">
                {deviceOnlyRecordings.map((recording) => (
                  <li key={recording.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-ink/10 bg-[#fbf7ef] px-3 py-2">
                    <span className="min-w-0">
                      <span className="block truncate font-display text-[13px] text-ink">{recording.topicName}</span>
                      <span className="text-[11px] text-ink/45">{new Date(recording.createdAt).toLocaleDateString()} · {fmtDuration(recording.durationSeconds)}</span>
                    </span>
                    <button
                      type="button"
                      onClick={() => void handleUploadToDeviceCopy(recording)}
                      disabled={uploadingId !== null}
                      className="rounded-lg bg-cobalt px-3 py-1.5 text-[12px] font-medium text-ivory disabled:opacity-50"
                    >
                      {uploadingId === recording.id ? (uploadProgress > 0 ? `Uploading… ${uploadProgress}%` : "Uploading…") : "Save to account"}
                    </button>
                  </li>
                ))}
              </ul>
          ...
          {cloudError ? <p className="mt-4 rounded-xl border border-[#d77f7f]/30 bg-[#f8e6e6] px-4 py-3 text-[13px] text-[#8b3c3c]">{cloudError}</p> : null}
